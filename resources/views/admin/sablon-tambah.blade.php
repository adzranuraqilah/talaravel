...
<div class="px-6 pt-6 max-w-2xl mx-auto">
    <!-- Header -->
    <div class="mb-6">
        <div class="text-2xl font-bold text-gray-800">Tambah Sablon</div>
        <div class="text-gray-500 text-sm">Tambah sablon baru</div>
    </div>
    
    <!-- Form Card -->
    <div class="bg-white border border-gray-200 rounded-xl p-6">
        <form action="{{ route('admin.sablon.store') }}" method="POST">
            @csrf
            <div class="space-y-4">
                <!-- Nama Sablon -->
                <div>
                    <label class="block text-gray-700 font-medium mb-2">Nama Sablon</label>
                    <input type="text" name="nama_sablon" placeholder="Masukkan nama sablon" value="{{ old('nama_sablon') }}" class="w-full border border-gray-300 rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#1e335f] focus:border-[#1e335f] @error('nama_sablon') border-red-500 @enderror" required />
                    @error('nama_sablon')
