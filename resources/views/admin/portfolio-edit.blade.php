...
<div class="flex items-center justify-center min-h-screen bg-gray-900">
    <div class="bg-white rounded-xl shadow p-8 w-full max-w-md">
        <h2 class="text-lg font-semibold text-gray-800 mb-6">Edit Portofolio</h2>
        <form action="{{ route('admin.portfolio.update', $item->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nama_produk" value="{{ $item->nama_produk }}" placeholder="Nama Portofolio" class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#1e335f] focus:border-[#1e335f] text-base placeholder-gray-400" />
            </div>
            <div class="mb-6">
                <label class="block text-gray-700 mb-1">Gambar Portofolio</label>
                @if($item->foto_pratinjau)
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $item->foto_pratinjau) }}" alt="{{ $item->nama_produk }}" class="w-32 h-32 object-cover rounded border">
                ...
                <label class="flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-lg p-6 text-center cursor-pointer hover:bg-gray-50 transition">
                    ...
                    <input type="file" name="foto_pratinjau" class="hidden" />
                </label>
            </div>
