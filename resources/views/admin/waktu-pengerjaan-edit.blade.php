<x-layout-admin>
    <div class="p-6">
        <div class="flex items-center gap-4 mb-6">
            ...
            <h1 class="text-2xl font-bold text-gray-900">Edit Waktu Pengerjaan</h1>
        </div>

        <div class="bg-white rounded-lg shadow p-6 max-w-2xl">
            <form action="{{ route('admin.waktu-pengerjaan.update', $waktuPengerjaan->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                        ...
                        <select name="produk_id" id="produk_id" 
                            ...
                            @foreach($produks as $produk)
                                <option value="{{ $produk->id }}" 
                                        {{ (old('produk_id', $waktuPengerjaan->produk_id) == $produk->id) ? 'selected' : '' }}>
                                    {{ $produk->nama_produk }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_waktu" id="nama_waktu" 
                               value="{{ old('nama_waktu', $waktuPengerjaan->nama_waktu) }}"
                               placeholder="Contoh: Standard, Express, Super Express"
                    ...
                    <div>
                        ...
                        <input type="number" name="durasi_hari" id="durasi_hari" 
                               value="{{ old('durasi_hari', $waktuPengerjaan->durasi_hari) }}"
                               min="1" max="365"
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" id="deskripsi" rows="4"
                                  ...
                                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $waktuPengerjaan->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
